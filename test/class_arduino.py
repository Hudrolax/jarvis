...
from modules import gfunctions as gf
from modules.class_pins import Pins
import serial
...
class Package:
    ...
    def __init__(self, body):
        if not isinstance(body, str):
            raise TypeError("Package: body is not 'str' type!")
        print(f'body: {body} type {type(body)}')
        self.body = body.encode(self.CODING)

if __name__ == '__main__':
    b = bytes(2)
    print(b)
    print(b'0x45'.decode('utf-8'))

class Arduino_COM(COM_port, Threaded_class, LoggerSuper):
    """Класс описывает коммуникацию по последовательному порту для Arduino"""
    def __init__(self, PID, speed, timeout):
        super().__init__(name='Mega', PID=PID, speed=speed, timeout=timeout)
        self._thread = threading.Thread(target=self._communicate_thread, args=(), daemon=True)
        self._thread.start()
        self._input_queue = Queue()
        self._output_queue = Queue()

    def _communicate_thread(self):
        """Функция описывает работу потока обмена по последовательному порту"""
        while self.working:
            pass

class Arduino(LoggerSuper):
    ...
    def set_pin(self, _pin, __state):
        if isinstance(_pin, list):
            self.logger.error("Error (set_pin): _pin is <class 'list'>")
            return None
        if __state:
            __state = 1
        elif not __state:
    ...
    def change_pinstate(self, pin, state):
        ...
        if p.state != p.prevstate:
            for b in p.binds:
                if not p.blocked and not b.blocked:
                    self.logger.info(f'im set pin {b.description} to {p.state} by pin_reaction in {datetime.now().strftime("%X")}')
    ...
    def write_pinstate(self, _pin):
        try:
            ...
            self.logger.error('error save pinstate')
                ...
                for p in self.pins:
                    if str(p.num) == s[0]:
                        ...
                        try:
                            ...
                            pass
            self.logger.info('pinstate is loaded')
        except:
            self.logger.error('error load pinstate')
    ...
    def prepare_serial(self):
        try:
            ...
            self.logger.error("Can't load proporties to COM port")
            return False
    ...
    def initialize(self):
        ...
        for p in ports:
            comport = p.device
            self.logger.info('Try to find Arduino in ' + comport)
            self.port = serial.Serial(comport, 57600, timeout=1)
            if self._try_to_init(): break

        if not self.initialized:
            self.logger.critical('I have not found the Arduino...')
            self.logger.critical("Sorry, but i can't work whithout Arduino subcontroller :(")
            self.logger.info("I'm have to try to find it after one second pause")
        else:
            self.logger.info(f'Arduino is initialized on port {self.port.name}')
            self.load_pinstate()
    ...
    def find_by_auction(self, cmd, allpins=False):
        ...
        for word in wordlist:
            for _pa in _pin_auction:
                self.logger.debug(f'count points for {_pa[0]}')
                for ct in _pa[1]:
                    if ct.lower() == word:
                        _pa[2] += 2
                        self.logger.debug(f'word = {word} 2 points')
                    elif ct.lower().find(word) > -1:
                        _pa[2] += 1
                        self.logger.debug(f'word find {word} 1 point')
        ...
        if len(_winners) == 1:
            self.logger.debug(f'winner is {_winners[0].description}')
            self.logger.debug(f'winner get {_max_includes} points')
            return _winners[0]
        elif len(_winners) > 1:
            self.logger.debug('_winners more than one')
            self.logger.debug(f'its get {_max_includes} points')
            return _winners
        else:
            self.logger.debug('winner not found')
            self.logger.debug('winner not found')
            return None
