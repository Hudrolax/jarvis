...
class Jarvis:
    ...
    def get_text_messages(self, message):
        ...
        if message.text == "привет":
            self.bot.reply_to(message, "Привет, чем я могу тебе помочь?")
            ...
            if _user is not None:
                ...
                self.bot.reply_to(message, helpmessage)
            else:
                self.bot.reply_to(message, "Кто ты чудовище?")
        elif message.text == 'getmyid':
            self.bot.reply_to(message, "Ваш ID: %s" % message.from_user.id)
            ...
            if _user is not None:
                self.input_queue.put((message.text, _user, message))  # Поместили сообщение в оцередь на обработку
                __answe_wait_time = 5
                while __answe_wait_time > 0:
                    if (self.telegram_answer_queue.qsize() > 0):
                        queue_typle = self.telegram_answer_queue.get()
                        ...
                        if get_message == message:
                            ...
                            self.telegram_answer_queue.put((get_message, answer))
    ...
    def handle_docs_audio(self, message):
        _user = None
        for user in self.bot.users:
            if str(message.from_user.id) == user.id:
                _user = user
        ...
        if _user != None:
            if message.content_type == 'sticker':
                ...
                self.bot.send_message(message.from_user.id, str(sticker_id))
            elif message.content_type == 'document':
                if message.document.file_name == 'config.txt':
                    file_info = self.bot.get_file(message.document.file_id)
                    downloaded_file = self.bot.download_file(file_info.file_path)
                    with open(self.arduino.config_path, 'wb') as new_file:
                        new_file.write(downloaded_file)
                    test_config = self.arduino.load_config(False)
                    if test_config == 'config loaded!':
                        self.bot.reply_to(message, "конфиг загрузил и применил")
                    else:
                        self.bot.reply_to(message, "почему-то не вышло загрузить конфиг")
                else:
